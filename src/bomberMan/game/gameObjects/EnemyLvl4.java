...
public class EnemyLvl4 extends Enemy implements Serializable {
    ...
    private final static int exploreTimes = 1;
    private final int radius = Math.min(gameBoard.getRows(),gameBoard.getColumns());
    private int exploreCount;
    private static double exploreVel;
    private static double chaseVel;

    private final static int standTimeBlocked = 10;
    private final static int standTimeChoosing = 0;
    private final static int standTimeLetGo = 10;
    private int standCounter;
    private Block origin;
    private Block destination;

    public EnemyLvl4(GameBoard gameBoard) {
        super(gameBoard);
        id = ObjectID.Enemylvl4;
        width = (int) (0.6*Block.getHeight());
        height = (int) (0.9*Block.getHeight());
        velX = defaultVelocity;
        velY = velX;
        animationSpeed = 0.1*velX;
        setUpAnimation("enemyLvl4");
    ...
    protected void move() {
        ...
        if (movingUp || movingDown || movingLeft || movingRight) {
            ...
            java.util.List<Block> options = new ArrayList<>();
            ...
            if (exploreCount >= exploreTimes){
                for (Player player : gameBoard.getPlayers()){
                    if (gameBoard.getDistance(player.getBlock(gameBoard),currentBlock) <= radius) {
                        chasing = true;
                        exploreCount = 0;
                        velX = velY = chaseVel;
                        break;
            ...
            if (!chasing) {
                ...
                if (distanceToPlayer > radius){
                    stand(standTimeLetGo);
                    chasing = false;
                    velX = velY = exploreVel;
                    return;
                }
